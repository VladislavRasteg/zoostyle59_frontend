import s from "./GroupRow.module.scss";
...
const GroupRow = ({ id, group_name, doctor_name, procedures, date, time, endTime, status }: ReceptionRowProps) => {
    ...
    return(
        <tr className={(status === 'Сеанс идет') ? `${s.trb} ${s.active}` : s.trb} onClick={() => navigate(RECEPTION_ROUTE + '/' + id)}>
            <td className={s.tdb}>{group_name}</td>
